...
public class APGARCH extends GARCH{
	...
	static double standError_asymmetryPower;
	static double tValue_asymmetryPower;
	...
	public static void do_MLE_4_GARCH_AP(){
    	...
    	if(optimizer == "DEoptim" || optimizer == "SANN"){
        	...
        	if(optimizer == "DEoptim"){
            	...
            	set_GARCH_AP_pars_from_vec(optimal_value );        	
    	...
    	hessian = NumDeriv.hessian(APGARCH::opti_log_likelihood_4_GARCH_AP, optimal_value, null);
    	estParCovariance = MatrixOperations.inverse(hessian);
    	set_GARCH_AP_standard_errors_and_t_values_of_est_pars();
	...
	public static void set_GARCH_AP_standard_errors_and_t_values_of_est_pars(){
		...
		for(int i=0; i<lag4residuals; i++){
			standErrors_asymmetryPars[i][0] = estParCovariance[idx][idx];
			tValues_asymmetryPars[i][0] = asymmetryPars[i][0]/standErrors_asymmetryPars[i][0]; 
			idx++;
		}
		
		standError_asymmetryPower = estParCovariance[idx][idx];
		tValue_asymmetryPower = asymmetryPower/standError_asymmetryPower;
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(standError_asymmetryPower);
