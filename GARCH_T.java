...
public class GARCH_T extends GARCH{
	...
	static double [][] standErrors_alpha_plus;
	static double [][] standErrors_alpha_minus;
	
	static double [][] tValues_alpha_plus;
	static double [][] tValues_alpha_minus;
	...
	public static double [][] calc_volatilies_from_GARCH_T(){
		...
		double [][] squared_h_t = new double [n_usedObservations][1];
		...
		for(int t=0; t<n_usedObservations; t++){
			...
			squared_h_t[t][0] = Math.pow(h_t[t][0],2.0);
		}
	...
	public static void do_MLE_4_GARCH_T(){
    	...
    	hessian = NumDeriv.hessian(GARCH_T::opti_log_likelihood_4_GARCH_T, optimal_value, null);
    	estParCovariance = MatrixOperations.inverse(hessian);
    	set_GARCH_T_standard_errors_and_t_values_of_est_pars();
	...
	public static void set_GARCH_T_standard_errors_and_t_values_of_est_pars(){
		
		standErrors_arPars      = new double [lag4observedVariables+1][1];
		standErrors_volaPars    = new double [lag4volatility+1][1];
		standErrors_alpha_plus  = new double [lag4residuals][1];
		standErrors_alpha_minus = new double [lag4residuals][1];
		
		tValues_arPars      = new double [lag4observedVariables+1][1];
		tValues_volaPars    = new double [lag4volatility+1][1];
		tValues_alpha_plus  = new double [lag4residuals][1];
		tValues_alpha_minus = new double [lag4residuals][1];
		...
		for(int i=0; i<lag4residuals; i++){
			standErrors_alpha_plus[i][0] = Math.sqrt(estParCovariance[idx][idx]);
			tValues_alpha_plus[i][0] = alpha_plus[i][0]/standErrors_alpha_plus[i][0];
			idx++;
		}
		
		for(int i=0; i<lag4residuals; i++){
			standErrors_alpha_minus[i][0] = Math.sqrt(estParCovariance[idx][idx]);
			tValues_alpha_minus[i][0] = alpha_minus[i][0]/standErrors_alpha_minus[i][0];
			idx++;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		MatrixOperations.print_matrix(standErrors_alpha_plus);
		MatrixOperations.print_matrix(standErrors_alpha_minus);	
