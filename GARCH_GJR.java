...
public class GARCH_GJR extends GARCH{
	...
	public GARCH_GJR(double[][] obs_variables, int start_idx, int end_idx, int obsLag, int volaLag, int resLag) {
		super(obs_variables, start_idx, end_idx, obsLag, volaLag, resLag);
		GARCH_model = "GJR-GARCH";
	}

	
	public static double [][] calc_volatilies_from_GARCH_GJR(){
		...
		for(int m=0; m<lag4residuals; m++){
			...
			for(int t=0; t<n_usedObservations; t++){
				...
				double u_indicator = 0.0;
				if(u_t >= 0.0){
					u_indicator = 1.0;
				}
				
				u_t = Math.pow(u_t,2.0);
				h_t[t][0] += (maPars[m][0]+asymmetryPars[m][0]*u_indicator)*u_t;		
			}			
		}
	...
	public static double opti_log_likelihood_4_GARCH_GJR(double [] pars, double [] further_ars){
		
		set_GARCH_GJR_pars_from_vec(pars);
		
		double logLik = calc_log_likelihood_4_GARCH_GJR();
	...
	public static double calc_log_likelihood_4_GARCH_GJR(){
		...
		double [][] h_t = calc_volatilies_from_GARCH_GJR();	
	...
	public static void do_MLE_4_GARCH_GJR(){
		
    	double [] start_value = get_start_values_4_est_GARCH_GJR();
    	...
    	if(optimizer == "DEoptim" || optimizer == "SANN"){
        	...
        	if(optimizer == "DEoptim"){
        		DifferentialEvolution optim = new DifferentialEvolution(GARCH_GJR::opti_log_likelihood_4_GARCH_GJR, 200);
            	...
            	set_GARCH_GJR_pars_from_vec(optimal_value);        	
            	logLikelihood = (-1.0)*optim.get_optimal_value();
            	convergence = optim.get_convergence_info();
        	}
    		
        	if(optimizer == "SANN"){
        		SimulatedAnnealing optim = new SimulatedAnnealing(GARCH_GJR::opti_log_likelihood_4_GARCH_GJR, 10000);
    	...
    	if(truePars == false){
    		System.out.println("GJR-GARCH restrictions for heteroscedasticity parameters violated.");
    	}
    	...
    	volatilities = calc_volatilies_from_GARCH_GJR();
    	fittedValues = calc_est_values_from_GARCH();
    	residuals = MatrixOperations.substract(obs_data, fittedValues);
    	
    	hessian = NumDeriv.hessian(GARCH_GJR::opti_log_likelihood_4_GARCH_GJR, optimal_value, null);
    	estParCovariance = MatrixOperations.inverse(hessian);
    	set_GARCH_GJR_standard_errors_and_t_values_of_est_pars();
	...
	public static void set_GARCH_GJR_standard_errors_and_t_values_of_est_pars(){
	...
	public static void set_GARCH_GJR_pars_from_vec(double [] par_vec){
	...
	public static double [] get_start_values_4_est_GARCH_GJR(){
		
		//Note: Start values for asymmetry parameters are set 0.0		
		int n_GARCH_pars = lag4observedVariables+lag4volatility+lag4residuals+2;
		int n_pars = n_GARCH_pars+lag4residuals;
		
		double [] start_values_4_GJR = new double [n_pars];		
		double [] start_values_4_GARCH = get_start_values_4_est_GARCH();
		
		for(int i=0; i<n_GARCH_pars; i++){
			start_values_4_GJR[i] = start_values_4_GARCH[i];
		}
		
		return start_values_4_GJR;
		
	}
	
	
	public static void get_GARCH_GJR_forecast_4_Gaussian(int n_steps){
		...
		for(int s=0; s<n_steps; s++){
			...
			for(int i=0; i<lag4residuals; i++){
				...
				if(histStep <=0){
					...
					u_t = Math.pow(u_t, 2.0);
					volatilityForecast[s][0] += (maPars[i][0]+asymmetryPars[i][0]*u_indicator)*u_t;
				}else{
					volatilityForecast[s][0] += (maPars[i][0]+asymmetryPars[i][0]/2.0)*volatilityForecast[(s-i-1)][0];
					idx++;
				}				
			}
	...
	public static void get_GARCH_GJR_forecast(int n_steps){
		
		if(distribution == "Normal"){
			get_GARCH_GJR_forecast_4_Gaussian(n_steps);
		}
	...
	public static void main(String[] args) throws Exception {
		...
		GARCH_GJR obj_arch = new GARCH_GJR(obsData, start_idx, end_idx, obsLag, volaLag, maLag);
		obj_arch.do_MLE_4_GARCH_GJR();
		obj_arch.get_GARCH_GJR_forecast_4_Gaussian(100);
