...
public class GARCH_Q extends GARCH{
	...
	public static double [][] calc_volatilies_from_GARCH_Q(){
		...
		for(int m=0; m<lag4residuals; m++){
			...
			for(int t=0; t<n_usedObservations; t++){
				...
				double u_t_squared = Math.pow(u_t,2.0);
				h_t[t][0] += maPars[m][0]*u_t_squared + linear_maPars[m][0]*u_t;
				
				//Check if h_t becomes negative.
				if(h_t[t][0]<0.0){
					return h_t;
				}
			}			
	...
	public static double calc_log_likelihood_4_GARCH_Q(){
		...
		double [][] h_t = calc_volatilies_from_GARCH_Q();		
	...
	public static double opti_log_likelihood_4_GARCH_Q(double [] pars, double [] further_ars){
		
		set_GARCH_Q_pars_from_vec(pars);
	...
	public static double [] get_start_values_4_est_GARCH_Q(){
		
		//Note: Start values for linear moving average parameters are set 0.0		
		int n_GARCH_pars = lag4observedVariables+lag4volatility+lag4residuals+2;
